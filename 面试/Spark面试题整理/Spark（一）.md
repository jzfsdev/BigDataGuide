...
* 了解 spark 内部的优化机制，针对使用场景正确的使用相关优化技术
* 编写代码的时候遵从 spark 的编码最佳实践，例如重复使用的数据加缓存，合理的使用 partition 和 bucketing，尽量减少 shuffling 的发生，减少 join 的使用
* 根据数据量和SLA，合理的使用系统资源
* 平台应该提高监控，能自动诊断出有潜在问题或者执行次数很多又有优化空间的任务
* [tuninig spark](http://spark.apache.org/docs/2.4.3/tuning.html)
